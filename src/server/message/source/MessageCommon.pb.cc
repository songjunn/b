// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: MessageCommon.proto

#define INTERNAL_SUPPRESS_PROTOBUF_FIELD_DEPRECATION
#include "MessageCommon.pb.h"

#include <algorithm>

#include <google/protobuf/stubs/common.h>
#include <google/protobuf/stubs/once.h>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/wire_format_lite_inl.h>
#include <google/protobuf/descriptor.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/reflection_ops.h>
#include <google/protobuf/wire_format.h>
// @@protoc_insertion_point(includes)

namespace Message {

namespace {

const ::google::protobuf::Descriptor* ErrorNo_descriptor_ = NULL;
const ::google::protobuf::internal::GeneratedMessageReflection*
  ErrorNo_reflection_ = NULL;
const ::google::protobuf::Descriptor* EventSync_descriptor_ = NULL;
const ::google::protobuf::internal::GeneratedMessageReflection*
  EventSync_reflection_ = NULL;

}  // namespace


void protobuf_AssignDesc_MessageCommon_2eproto() {
  protobuf_AddDesc_MessageCommon_2eproto();
  const ::google::protobuf::FileDescriptor* file =
    ::google::protobuf::DescriptorPool::generated_pool()->FindFileByName(
      "MessageCommon.proto");
  GOOGLE_CHECK(file != NULL);
  ErrorNo_descriptor_ = file->message_type(0);
  static const int ErrorNo_offsets_[12] = {
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, error_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, userid_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, type_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramint1_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramint2_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramint3_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramlong1_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramlong2_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramlong3_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramstr1_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramstr2_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, paramstr3_),
  };
  ErrorNo_reflection_ =
    new ::google::protobuf::internal::GeneratedMessageReflection(
      ErrorNo_descriptor_,
      ErrorNo::default_instance_,
      ErrorNo_offsets_,
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, _has_bits_[0]),
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(ErrorNo, _unknown_fields_),
      -1,
      ::google::protobuf::DescriptorPool::generated_pool(),
      ::google::protobuf::MessageFactory::generated_factory(),
      sizeof(ErrorNo));
  EventSync_descriptor_ = file->message_type(1);
  static const int EventSync_offsets_[11] = {
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, evid_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, pid_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param1_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param2_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param3_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param4_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param5_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param6_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, param7_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, stime_),
    GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, parent_),
  };
  EventSync_reflection_ =
    new ::google::protobuf::internal::GeneratedMessageReflection(
      EventSync_descriptor_,
      EventSync::default_instance_,
      EventSync_offsets_,
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, _has_bits_[0]),
      GOOGLE_PROTOBUF_GENERATED_MESSAGE_FIELD_OFFSET(EventSync, _unknown_fields_),
      -1,
      ::google::protobuf::DescriptorPool::generated_pool(),
      ::google::protobuf::MessageFactory::generated_factory(),
      sizeof(EventSync));
}

namespace {

GOOGLE_PROTOBUF_DECLARE_ONCE(protobuf_AssignDescriptors_once_);
inline void protobuf_AssignDescriptorsOnce() {
  ::google::protobuf::GoogleOnceInit(&protobuf_AssignDescriptors_once_,
                 &protobuf_AssignDesc_MessageCommon_2eproto);
}

void protobuf_RegisterTypes(const ::std::string&) {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedMessage(
    ErrorNo_descriptor_, &ErrorNo::default_instance());
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedMessage(
    EventSync_descriptor_, &EventSync::default_instance());
}

}  // namespace

void protobuf_ShutdownFile_MessageCommon_2eproto() {
  delete ErrorNo::default_instance_;
  delete ErrorNo_reflection_;
  delete EventSync::default_instance_;
  delete EventSync_reflection_;
}

void protobuf_AddDesc_MessageCommon_2eproto() {
  static bool already_here = false;
  if (already_here) return;
  already_here = true;
  GOOGLE_PROTOBUF_VERIFY_VERSION;

  ::google::protobuf::DescriptorPool::InternalAddGeneratedFile(
    "\n\023MessageCommon.proto\022\007Message\"\344\001\n\007Error"
    "No\022\r\n\005error\030\001 \001(\005\022\016\n\006userid\030\002 \001(\003\022\014\n\004typ"
    "e\030\003 \001(\005\022\021\n\tparamint1\030\004 \001(\005\022\021\n\tparamint2\030"
    "\005 \001(\005\022\021\n\tparamint3\030\006 \001(\005\022\022\n\nparamlong1\030\007"
    " \001(\003\022\022\n\nparamlong2\030\010 \001(\003\022\022\n\nparamlong3\030\t"
    " \001(\003\022\021\n\tparamstr1\030\n \001(\t\022\021\n\tparamstr2\030\013 \001"
    "(\t\022\021\n\tparamstr3\030\014 \001(\t\"\265\001\n\tEventSync\022\014\n\004e"
    "vid\030\001 \001(\005\022\013\n\003pid\030\002 \001(\003\022\016\n\006param1\030\003 \001(\003\022\016"
    "\n\006param2\030\004 \001(\003\022\016\n\006param3\030\005 \001(\003\022\016\n\006param4"
    "\030\006 \001(\003\022\016\n\006param5\030\007 \001(\003\022\016\n\006param6\030\010 \001(\003\022\016"
    "\n\006param7\030\t \001(\t\022\r\n\005stime\030\n \001(\t\022\016\n\006parent\030"
    "\013 \001(\003", 445);
  ::google::protobuf::MessageFactory::InternalRegisterGeneratedFile(
    "MessageCommon.proto", &protobuf_RegisterTypes);
  ErrorNo::default_instance_ = new ErrorNo();
  EventSync::default_instance_ = new EventSync();
  ErrorNo::default_instance_->InitAsDefaultInstance();
  EventSync::default_instance_->InitAsDefaultInstance();
  ::google::protobuf::internal::OnShutdown(&protobuf_ShutdownFile_MessageCommon_2eproto);
}

// Force AddDescriptors() to be called at static initialization time.
struct StaticDescriptorInitializer_MessageCommon_2eproto {
  StaticDescriptorInitializer_MessageCommon_2eproto() {
    protobuf_AddDesc_MessageCommon_2eproto();
  }
} static_descriptor_initializer_MessageCommon_2eproto_;

// ===================================================================

#ifndef _MSC_VER
const int ErrorNo::kErrorFieldNumber;
const int ErrorNo::kUseridFieldNumber;
const int ErrorNo::kTypeFieldNumber;
const int ErrorNo::kParamint1FieldNumber;
const int ErrorNo::kParamint2FieldNumber;
const int ErrorNo::kParamint3FieldNumber;
const int ErrorNo::kParamlong1FieldNumber;
const int ErrorNo::kParamlong2FieldNumber;
const int ErrorNo::kParamlong3FieldNumber;
const int ErrorNo::kParamstr1FieldNumber;
const int ErrorNo::kParamstr2FieldNumber;
const int ErrorNo::kParamstr3FieldNumber;
#endif  // !_MSC_VER

ErrorNo::ErrorNo()
  : ::google::protobuf::Message() {
  SharedCtor();
  // @@protoc_insertion_point(constructor:Message.ErrorNo)
}

void ErrorNo::InitAsDefaultInstance() {
}

ErrorNo::ErrorNo(const ErrorNo& from)
  : ::google::protobuf::Message() {
  SharedCtor();
  MergeFrom(from);
  // @@protoc_insertion_point(copy_constructor:Message.ErrorNo)
}

void ErrorNo::SharedCtor() {
  ::google::protobuf::internal::GetEmptyString();
  _cached_size_ = 0;
  error_ = 0;
  userid_ = GOOGLE_LONGLONG(0);
  type_ = 0;
  paramint1_ = 0;
  paramint2_ = 0;
  paramint3_ = 0;
  paramlong1_ = GOOGLE_LONGLONG(0);
  paramlong2_ = GOOGLE_LONGLONG(0);
  paramlong3_ = GOOGLE_LONGLONG(0);
  paramstr1_ = const_cast< ::std::string*>(&::google::protobuf::internal::GetEmptyStringAlreadyInited());
  paramstr2_ = const_cast< ::std::string*>(&::google::protobuf::internal::GetEmptyStringAlreadyInited());
  paramstr3_ = const_cast< ::std::string*>(&::google::protobuf::internal::GetEmptyStringAlreadyInited());
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}

ErrorNo::~ErrorNo() {
  // @@protoc_insertion_point(destructor:Message.ErrorNo)
  SharedDtor();
}

void ErrorNo::SharedDtor() {
  if (paramstr1_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
    delete paramstr1_;
  }
  if (paramstr2_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
    delete paramstr2_;
  }
  if (paramstr3_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
    delete paramstr3_;
  }
  if (this != default_instance_) {
  }
}

void ErrorNo::SetCachedSize(int size) const {
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
}
const ::google::protobuf::Descriptor* ErrorNo::descriptor() {
  protobuf_AssignDescriptorsOnce();
  return ErrorNo_descriptor_;
}

const ErrorNo& ErrorNo::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_MessageCommon_2eproto();
  return *default_instance_;
}

ErrorNo* ErrorNo::default_instance_ = NULL;

ErrorNo* ErrorNo::New() const {
  return new ErrorNo;
}

void ErrorNo::Clear() {
#define OFFSET_OF_FIELD_(f) (reinterpret_cast<char*>(      \
  &reinterpret_cast<ErrorNo*>(16)->f) - \
   reinterpret_cast<char*>(16))

#define ZR_(first, last) do {                              \
    size_t f = OFFSET_OF_FIELD_(first);                    \
    size_t n = OFFSET_OF_FIELD_(last) - f + sizeof(last);  \
    ::memset(&first, 0, n);                                \
  } while (0)

  if (_has_bits_[0 / 32] & 255) {
    ZR_(userid_, paramlong2_);
    paramint3_ = 0;
  }
  if (_has_bits_[8 / 32] & 3840) {
    paramlong3_ = GOOGLE_LONGLONG(0);
    if (has_paramstr1()) {
      if (paramstr1_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        paramstr1_->clear();
      }
    }
    if (has_paramstr2()) {
      if (paramstr2_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        paramstr2_->clear();
      }
    }
    if (has_paramstr3()) {
      if (paramstr3_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        paramstr3_->clear();
      }
    }
  }

#undef OFFSET_OF_FIELD_
#undef ZR_

  ::memset(_has_bits_, 0, sizeof(_has_bits_));
  mutable_unknown_fields()->Clear();
}

bool ErrorNo::MergePartialFromCodedStream(
    ::google::protobuf::io::CodedInputStream* input) {
#define DO_(EXPRESSION) if (!(EXPRESSION)) goto failure
  ::google::protobuf::uint32 tag;
  // @@protoc_insertion_point(parse_start:Message.ErrorNo)
  for (;;) {
    ::std::pair< ::google::protobuf::uint32, bool> p = input->ReadTagWithCutoff(127);
    tag = p.first;
    if (!p.second) goto handle_unusual;
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      // optional int32 error = 1;
      case 1: {
        if (tag == 8) {
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &error_)));
          set_has_error();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(16)) goto parse_userid;
        break;
      }

      // optional int64 userid = 2;
      case 2: {
        if (tag == 16) {
         parse_userid:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &userid_)));
          set_has_userid();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(24)) goto parse_type;
        break;
      }

      // optional int32 type = 3;
      case 3: {
        if (tag == 24) {
         parse_type:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &type_)));
          set_has_type();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(32)) goto parse_paramint1;
        break;
      }

      // optional int32 paramint1 = 4;
      case 4: {
        if (tag == 32) {
         parse_paramint1:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &paramint1_)));
          set_has_paramint1();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(40)) goto parse_paramint2;
        break;
      }

      // optional int32 paramint2 = 5;
      case 5: {
        if (tag == 40) {
         parse_paramint2:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &paramint2_)));
          set_has_paramint2();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(48)) goto parse_paramint3;
        break;
      }

      // optional int32 paramint3 = 6;
      case 6: {
        if (tag == 48) {
         parse_paramint3:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &paramint3_)));
          set_has_paramint3();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(56)) goto parse_paramlong1;
        break;
      }

      // optional int64 paramlong1 = 7;
      case 7: {
        if (tag == 56) {
         parse_paramlong1:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &paramlong1_)));
          set_has_paramlong1();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(64)) goto parse_paramlong2;
        break;
      }

      // optional int64 paramlong2 = 8;
      case 8: {
        if (tag == 64) {
         parse_paramlong2:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &paramlong2_)));
          set_has_paramlong2();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(72)) goto parse_paramlong3;
        break;
      }

      // optional int64 paramlong3 = 9;
      case 9: {
        if (tag == 72) {
         parse_paramlong3:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &paramlong3_)));
          set_has_paramlong3();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(82)) goto parse_paramstr1;
        break;
      }

      // optional string paramstr1 = 10;
      case 10: {
        if (tag == 82) {
         parse_paramstr1:
          DO_(::google::protobuf::internal::WireFormatLite::ReadString(
                input, this->mutable_paramstr1()));
          ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
            this->paramstr1().data(), this->paramstr1().length(),
            ::google::protobuf::internal::WireFormat::PARSE,
            "paramstr1");
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(90)) goto parse_paramstr2;
        break;
      }

      // optional string paramstr2 = 11;
      case 11: {
        if (tag == 90) {
         parse_paramstr2:
          DO_(::google::protobuf::internal::WireFormatLite::ReadString(
                input, this->mutable_paramstr2()));
          ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
            this->paramstr2().data(), this->paramstr2().length(),
            ::google::protobuf::internal::WireFormat::PARSE,
            "paramstr2");
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(98)) goto parse_paramstr3;
        break;
      }

      // optional string paramstr3 = 12;
      case 12: {
        if (tag == 98) {
         parse_paramstr3:
          DO_(::google::protobuf::internal::WireFormatLite::ReadString(
                input, this->mutable_paramstr3()));
          ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
            this->paramstr3().data(), this->paramstr3().length(),
            ::google::protobuf::internal::WireFormat::PARSE,
            "paramstr3");
        } else {
          goto handle_unusual;
        }
        if (input->ExpectAtEnd()) goto success;
        break;
      }

      default: {
      handle_unusual:
        if (tag == 0 ||
            ::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_END_GROUP) {
          goto success;
        }
        DO_(::google::protobuf::internal::WireFormat::SkipField(
              input, tag, mutable_unknown_fields()));
        break;
      }
    }
  }
success:
  // @@protoc_insertion_point(parse_success:Message.ErrorNo)
  return true;
failure:
  // @@protoc_insertion_point(parse_failure:Message.ErrorNo)
  return false;
#undef DO_
}

void ErrorNo::SerializeWithCachedSizes(
    ::google::protobuf::io::CodedOutputStream* output) const {
  // @@protoc_insertion_point(serialize_start:Message.ErrorNo)
  // optional int32 error = 1;
  if (has_error()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(1, this->error(), output);
  }

  // optional int64 userid = 2;
  if (has_userid()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(2, this->userid(), output);
  }

  // optional int32 type = 3;
  if (has_type()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(3, this->type(), output);
  }

  // optional int32 paramint1 = 4;
  if (has_paramint1()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(4, this->paramint1(), output);
  }

  // optional int32 paramint2 = 5;
  if (has_paramint2()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(5, this->paramint2(), output);
  }

  // optional int32 paramint3 = 6;
  if (has_paramint3()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(6, this->paramint3(), output);
  }

  // optional int64 paramlong1 = 7;
  if (has_paramlong1()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(7, this->paramlong1(), output);
  }

  // optional int64 paramlong2 = 8;
  if (has_paramlong2()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(8, this->paramlong2(), output);
  }

  // optional int64 paramlong3 = 9;
  if (has_paramlong3()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(9, this->paramlong3(), output);
  }

  // optional string paramstr1 = 10;
  if (has_paramstr1()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr1().data(), this->paramstr1().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr1");
    ::google::protobuf::internal::WireFormatLite::WriteStringMaybeAliased(
      10, this->paramstr1(), output);
  }

  // optional string paramstr2 = 11;
  if (has_paramstr2()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr2().data(), this->paramstr2().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr2");
    ::google::protobuf::internal::WireFormatLite::WriteStringMaybeAliased(
      11, this->paramstr2(), output);
  }

  // optional string paramstr3 = 12;
  if (has_paramstr3()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr3().data(), this->paramstr3().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr3");
    ::google::protobuf::internal::WireFormatLite::WriteStringMaybeAliased(
      12, this->paramstr3(), output);
  }

  if (!unknown_fields().empty()) {
    ::google::protobuf::internal::WireFormat::SerializeUnknownFields(
        unknown_fields(), output);
  }
  // @@protoc_insertion_point(serialize_end:Message.ErrorNo)
}

::google::protobuf::uint8* ErrorNo::SerializeWithCachedSizesToArray(
    ::google::protobuf::uint8* target) const {
  // @@protoc_insertion_point(serialize_to_array_start:Message.ErrorNo)
  // optional int32 error = 1;
  if (has_error()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(1, this->error(), target);
  }

  // optional int64 userid = 2;
  if (has_userid()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(2, this->userid(), target);
  }

  // optional int32 type = 3;
  if (has_type()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(3, this->type(), target);
  }

  // optional int32 paramint1 = 4;
  if (has_paramint1()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(4, this->paramint1(), target);
  }

  // optional int32 paramint2 = 5;
  if (has_paramint2()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(5, this->paramint2(), target);
  }

  // optional int32 paramint3 = 6;
  if (has_paramint3()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(6, this->paramint3(), target);
  }

  // optional int64 paramlong1 = 7;
  if (has_paramlong1()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(7, this->paramlong1(), target);
  }

  // optional int64 paramlong2 = 8;
  if (has_paramlong2()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(8, this->paramlong2(), target);
  }

  // optional int64 paramlong3 = 9;
  if (has_paramlong3()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(9, this->paramlong3(), target);
  }

  // optional string paramstr1 = 10;
  if (has_paramstr1()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr1().data(), this->paramstr1().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr1");
    target =
      ::google::protobuf::internal::WireFormatLite::WriteStringToArray(
        10, this->paramstr1(), target);
  }

  // optional string paramstr2 = 11;
  if (has_paramstr2()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr2().data(), this->paramstr2().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr2");
    target =
      ::google::protobuf::internal::WireFormatLite::WriteStringToArray(
        11, this->paramstr2(), target);
  }

  // optional string paramstr3 = 12;
  if (has_paramstr3()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->paramstr3().data(), this->paramstr3().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "paramstr3");
    target =
      ::google::protobuf::internal::WireFormatLite::WriteStringToArray(
        12, this->paramstr3(), target);
  }

  if (!unknown_fields().empty()) {
    target = ::google::protobuf::internal::WireFormat::SerializeUnknownFieldsToArray(
        unknown_fields(), target);
  }
  // @@protoc_insertion_point(serialize_to_array_end:Message.ErrorNo)
  return target;
}

int ErrorNo::ByteSize() const {
  int total_size = 0;

  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    // optional int32 error = 1;
    if (has_error()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->error());
    }

    // optional int64 userid = 2;
    if (has_userid()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->userid());
    }

    // optional int32 type = 3;
    if (has_type()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->type());
    }

    // optional int32 paramint1 = 4;
    if (has_paramint1()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->paramint1());
    }

    // optional int32 paramint2 = 5;
    if (has_paramint2()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->paramint2());
    }

    // optional int32 paramint3 = 6;
    if (has_paramint3()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->paramint3());
    }

    // optional int64 paramlong1 = 7;
    if (has_paramlong1()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->paramlong1());
    }

    // optional int64 paramlong2 = 8;
    if (has_paramlong2()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->paramlong2());
    }

  }
  if (_has_bits_[8 / 32] & (0xffu << (8 % 32))) {
    // optional int64 paramlong3 = 9;
    if (has_paramlong3()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->paramlong3());
    }

    // optional string paramstr1 = 10;
    if (has_paramstr1()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::StringSize(
          this->paramstr1());
    }

    // optional string paramstr2 = 11;
    if (has_paramstr2()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::StringSize(
          this->paramstr2());
    }

    // optional string paramstr3 = 12;
    if (has_paramstr3()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::StringSize(
          this->paramstr3());
    }

  }
  if (!unknown_fields().empty()) {
    total_size +=
      ::google::protobuf::internal::WireFormat::ComputeUnknownFieldsSize(
        unknown_fields());
  }
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = total_size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
  return total_size;
}

void ErrorNo::MergeFrom(const ::google::protobuf::Message& from) {
  GOOGLE_CHECK_NE(&from, this);
  const ErrorNo* source =
    ::google::protobuf::internal::dynamic_cast_if_available<const ErrorNo*>(
      &from);
  if (source == NULL) {
    ::google::protobuf::internal::ReflectionOps::Merge(from, this);
  } else {
    MergeFrom(*source);
  }
}

void ErrorNo::MergeFrom(const ErrorNo& from) {
  GOOGLE_CHECK_NE(&from, this);
  if (from._has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    if (from.has_error()) {
      set_error(from.error());
    }
    if (from.has_userid()) {
      set_userid(from.userid());
    }
    if (from.has_type()) {
      set_type(from.type());
    }
    if (from.has_paramint1()) {
      set_paramint1(from.paramint1());
    }
    if (from.has_paramint2()) {
      set_paramint2(from.paramint2());
    }
    if (from.has_paramint3()) {
      set_paramint3(from.paramint3());
    }
    if (from.has_paramlong1()) {
      set_paramlong1(from.paramlong1());
    }
    if (from.has_paramlong2()) {
      set_paramlong2(from.paramlong2());
    }
  }
  if (from._has_bits_[8 / 32] & (0xffu << (8 % 32))) {
    if (from.has_paramlong3()) {
      set_paramlong3(from.paramlong3());
    }
    if (from.has_paramstr1()) {
      set_paramstr1(from.paramstr1());
    }
    if (from.has_paramstr2()) {
      set_paramstr2(from.paramstr2());
    }
    if (from.has_paramstr3()) {
      set_paramstr3(from.paramstr3());
    }
  }
  mutable_unknown_fields()->MergeFrom(from.unknown_fields());
}

void ErrorNo::CopyFrom(const ::google::protobuf::Message& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

void ErrorNo::CopyFrom(const ErrorNo& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool ErrorNo::IsInitialized() const {

  return true;
}

void ErrorNo::Swap(ErrorNo* other) {
  if (other != this) {
    std::swap(error_, other->error_);
    std::swap(userid_, other->userid_);
    std::swap(type_, other->type_);
    std::swap(paramint1_, other->paramint1_);
    std::swap(paramint2_, other->paramint2_);
    std::swap(paramint3_, other->paramint3_);
    std::swap(paramlong1_, other->paramlong1_);
    std::swap(paramlong2_, other->paramlong2_);
    std::swap(paramlong3_, other->paramlong3_);
    std::swap(paramstr1_, other->paramstr1_);
    std::swap(paramstr2_, other->paramstr2_);
    std::swap(paramstr3_, other->paramstr3_);
    std::swap(_has_bits_[0], other->_has_bits_[0]);
    _unknown_fields_.Swap(&other->_unknown_fields_);
    std::swap(_cached_size_, other->_cached_size_);
  }
}

::google::protobuf::Metadata ErrorNo::GetMetadata() const {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::Metadata metadata;
  metadata.descriptor = ErrorNo_descriptor_;
  metadata.reflection = ErrorNo_reflection_;
  return metadata;
}


// ===================================================================

#ifndef _MSC_VER
const int EventSync::kEvidFieldNumber;
const int EventSync::kPidFieldNumber;
const int EventSync::kParam1FieldNumber;
const int EventSync::kParam2FieldNumber;
const int EventSync::kParam3FieldNumber;
const int EventSync::kParam4FieldNumber;
const int EventSync::kParam5FieldNumber;
const int EventSync::kParam6FieldNumber;
const int EventSync::kParam7FieldNumber;
const int EventSync::kStimeFieldNumber;
const int EventSync::kParentFieldNumber;
#endif  // !_MSC_VER

EventSync::EventSync()
  : ::google::protobuf::Message() {
  SharedCtor();
  // @@protoc_insertion_point(constructor:Message.EventSync)
}

void EventSync::InitAsDefaultInstance() {
}

EventSync::EventSync(const EventSync& from)
  : ::google::protobuf::Message() {
  SharedCtor();
  MergeFrom(from);
  // @@protoc_insertion_point(copy_constructor:Message.EventSync)
}

void EventSync::SharedCtor() {
  ::google::protobuf::internal::GetEmptyString();
  _cached_size_ = 0;
  evid_ = 0;
  pid_ = GOOGLE_LONGLONG(0);
  param1_ = GOOGLE_LONGLONG(0);
  param2_ = GOOGLE_LONGLONG(0);
  param3_ = GOOGLE_LONGLONG(0);
  param4_ = GOOGLE_LONGLONG(0);
  param5_ = GOOGLE_LONGLONG(0);
  param6_ = GOOGLE_LONGLONG(0);
  param7_ = const_cast< ::std::string*>(&::google::protobuf::internal::GetEmptyStringAlreadyInited());
  stime_ = const_cast< ::std::string*>(&::google::protobuf::internal::GetEmptyStringAlreadyInited());
  parent_ = GOOGLE_LONGLONG(0);
  ::memset(_has_bits_, 0, sizeof(_has_bits_));
}

EventSync::~EventSync() {
  // @@protoc_insertion_point(destructor:Message.EventSync)
  SharedDtor();
}

void EventSync::SharedDtor() {
  if (param7_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
    delete param7_;
  }
  if (stime_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
    delete stime_;
  }
  if (this != default_instance_) {
  }
}

void EventSync::SetCachedSize(int size) const {
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
}
const ::google::protobuf::Descriptor* EventSync::descriptor() {
  protobuf_AssignDescriptorsOnce();
  return EventSync_descriptor_;
}

const EventSync& EventSync::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_MessageCommon_2eproto();
  return *default_instance_;
}

EventSync* EventSync::default_instance_ = NULL;

EventSync* EventSync::New() const {
  return new EventSync;
}

void EventSync::Clear() {
#define OFFSET_OF_FIELD_(f) (reinterpret_cast<char*>(      \
  &reinterpret_cast<EventSync*>(16)->f) - \
   reinterpret_cast<char*>(16))

#define ZR_(first, last) do {                              \
    size_t f = OFFSET_OF_FIELD_(first);                    \
    size_t n = OFFSET_OF_FIELD_(last) - f + sizeof(last);  \
    ::memset(&first, 0, n);                                \
  } while (0)

  if (_has_bits_[0 / 32] & 255) {
    ZR_(pid_, param6_);
    evid_ = 0;
  }
  if (_has_bits_[8 / 32] & 1792) {
    if (has_param7()) {
      if (param7_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        param7_->clear();
      }
    }
    if (has_stime()) {
      if (stime_ != &::google::protobuf::internal::GetEmptyStringAlreadyInited()) {
        stime_->clear();
      }
    }
    parent_ = GOOGLE_LONGLONG(0);
  }

#undef OFFSET_OF_FIELD_
#undef ZR_

  ::memset(_has_bits_, 0, sizeof(_has_bits_));
  mutable_unknown_fields()->Clear();
}

bool EventSync::MergePartialFromCodedStream(
    ::google::protobuf::io::CodedInputStream* input) {
#define DO_(EXPRESSION) if (!(EXPRESSION)) goto failure
  ::google::protobuf::uint32 tag;
  // @@protoc_insertion_point(parse_start:Message.EventSync)
  for (;;) {
    ::std::pair< ::google::protobuf::uint32, bool> p = input->ReadTagWithCutoff(127);
    tag = p.first;
    if (!p.second) goto handle_unusual;
    switch (::google::protobuf::internal::WireFormatLite::GetTagFieldNumber(tag)) {
      // optional int32 evid = 1;
      case 1: {
        if (tag == 8) {
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int32, ::google::protobuf::internal::WireFormatLite::TYPE_INT32>(
                 input, &evid_)));
          set_has_evid();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(16)) goto parse_pid;
        break;
      }

      // optional int64 pid = 2;
      case 2: {
        if (tag == 16) {
         parse_pid:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &pid_)));
          set_has_pid();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(24)) goto parse_param1;
        break;
      }

      // optional int64 param1 = 3;
      case 3: {
        if (tag == 24) {
         parse_param1:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param1_)));
          set_has_param1();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(32)) goto parse_param2;
        break;
      }

      // optional int64 param2 = 4;
      case 4: {
        if (tag == 32) {
         parse_param2:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param2_)));
          set_has_param2();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(40)) goto parse_param3;
        break;
      }

      // optional int64 param3 = 5;
      case 5: {
        if (tag == 40) {
         parse_param3:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param3_)));
          set_has_param3();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(48)) goto parse_param4;
        break;
      }

      // optional int64 param4 = 6;
      case 6: {
        if (tag == 48) {
         parse_param4:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param4_)));
          set_has_param4();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(56)) goto parse_param5;
        break;
      }

      // optional int64 param5 = 7;
      case 7: {
        if (tag == 56) {
         parse_param5:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param5_)));
          set_has_param5();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(64)) goto parse_param6;
        break;
      }

      // optional int64 param6 = 8;
      case 8: {
        if (tag == 64) {
         parse_param6:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &param6_)));
          set_has_param6();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(74)) goto parse_param7;
        break;
      }

      // optional string param7 = 9;
      case 9: {
        if (tag == 74) {
         parse_param7:
          DO_(::google::protobuf::internal::WireFormatLite::ReadString(
                input, this->mutable_param7()));
          ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
            this->param7().data(), this->param7().length(),
            ::google::protobuf::internal::WireFormat::PARSE,
            "param7");
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(82)) goto parse_stime;
        break;
      }

      // optional string stime = 10;
      case 10: {
        if (tag == 82) {
         parse_stime:
          DO_(::google::protobuf::internal::WireFormatLite::ReadString(
                input, this->mutable_stime()));
          ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
            this->stime().data(), this->stime().length(),
            ::google::protobuf::internal::WireFormat::PARSE,
            "stime");
        } else {
          goto handle_unusual;
        }
        if (input->ExpectTag(88)) goto parse_parent;
        break;
      }

      // optional int64 parent = 11;
      case 11: {
        if (tag == 88) {
         parse_parent:
          DO_((::google::protobuf::internal::WireFormatLite::ReadPrimitive<
                   ::google::protobuf::int64, ::google::protobuf::internal::WireFormatLite::TYPE_INT64>(
                 input, &parent_)));
          set_has_parent();
        } else {
          goto handle_unusual;
        }
        if (input->ExpectAtEnd()) goto success;
        break;
      }

      default: {
      handle_unusual:
        if (tag == 0 ||
            ::google::protobuf::internal::WireFormatLite::GetTagWireType(tag) ==
            ::google::protobuf::internal::WireFormatLite::WIRETYPE_END_GROUP) {
          goto success;
        }
        DO_(::google::protobuf::internal::WireFormat::SkipField(
              input, tag, mutable_unknown_fields()));
        break;
      }
    }
  }
success:
  // @@protoc_insertion_point(parse_success:Message.EventSync)
  return true;
failure:
  // @@protoc_insertion_point(parse_failure:Message.EventSync)
  return false;
#undef DO_
}

void EventSync::SerializeWithCachedSizes(
    ::google::protobuf::io::CodedOutputStream* output) const {
  // @@protoc_insertion_point(serialize_start:Message.EventSync)
  // optional int32 evid = 1;
  if (has_evid()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt32(1, this->evid(), output);
  }

  // optional int64 pid = 2;
  if (has_pid()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(2, this->pid(), output);
  }

  // optional int64 param1 = 3;
  if (has_param1()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(3, this->param1(), output);
  }

  // optional int64 param2 = 4;
  if (has_param2()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(4, this->param2(), output);
  }

  // optional int64 param3 = 5;
  if (has_param3()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(5, this->param3(), output);
  }

  // optional int64 param4 = 6;
  if (has_param4()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(6, this->param4(), output);
  }

  // optional int64 param5 = 7;
  if (has_param5()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(7, this->param5(), output);
  }

  // optional int64 param6 = 8;
  if (has_param6()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(8, this->param6(), output);
  }

  // optional string param7 = 9;
  if (has_param7()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->param7().data(), this->param7().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "param7");
    ::google::protobuf::internal::WireFormatLite::WriteStringMaybeAliased(
      9, this->param7(), output);
  }

  // optional string stime = 10;
  if (has_stime()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->stime().data(), this->stime().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "stime");
    ::google::protobuf::internal::WireFormatLite::WriteStringMaybeAliased(
      10, this->stime(), output);
  }

  // optional int64 parent = 11;
  if (has_parent()) {
    ::google::protobuf::internal::WireFormatLite::WriteInt64(11, this->parent(), output);
  }

  if (!unknown_fields().empty()) {
    ::google::protobuf::internal::WireFormat::SerializeUnknownFields(
        unknown_fields(), output);
  }
  // @@protoc_insertion_point(serialize_end:Message.EventSync)
}

::google::protobuf::uint8* EventSync::SerializeWithCachedSizesToArray(
    ::google::protobuf::uint8* target) const {
  // @@protoc_insertion_point(serialize_to_array_start:Message.EventSync)
  // optional int32 evid = 1;
  if (has_evid()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt32ToArray(1, this->evid(), target);
  }

  // optional int64 pid = 2;
  if (has_pid()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(2, this->pid(), target);
  }

  // optional int64 param1 = 3;
  if (has_param1()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(3, this->param1(), target);
  }

  // optional int64 param2 = 4;
  if (has_param2()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(4, this->param2(), target);
  }

  // optional int64 param3 = 5;
  if (has_param3()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(5, this->param3(), target);
  }

  // optional int64 param4 = 6;
  if (has_param4()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(6, this->param4(), target);
  }

  // optional int64 param5 = 7;
  if (has_param5()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(7, this->param5(), target);
  }

  // optional int64 param6 = 8;
  if (has_param6()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(8, this->param6(), target);
  }

  // optional string param7 = 9;
  if (has_param7()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->param7().data(), this->param7().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "param7");
    target =
      ::google::protobuf::internal::WireFormatLite::WriteStringToArray(
        9, this->param7(), target);
  }

  // optional string stime = 10;
  if (has_stime()) {
    ::google::protobuf::internal::WireFormat::VerifyUTF8StringNamedField(
      this->stime().data(), this->stime().length(),
      ::google::protobuf::internal::WireFormat::SERIALIZE,
      "stime");
    target =
      ::google::protobuf::internal::WireFormatLite::WriteStringToArray(
        10, this->stime(), target);
  }

  // optional int64 parent = 11;
  if (has_parent()) {
    target = ::google::protobuf::internal::WireFormatLite::WriteInt64ToArray(11, this->parent(), target);
  }

  if (!unknown_fields().empty()) {
    target = ::google::protobuf::internal::WireFormat::SerializeUnknownFieldsToArray(
        unknown_fields(), target);
  }
  // @@protoc_insertion_point(serialize_to_array_end:Message.EventSync)
  return target;
}

int EventSync::ByteSize() const {
  int total_size = 0;

  if (_has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    // optional int32 evid = 1;
    if (has_evid()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int32Size(
          this->evid());
    }

    // optional int64 pid = 2;
    if (has_pid()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->pid());
    }

    // optional int64 param1 = 3;
    if (has_param1()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param1());
    }

    // optional int64 param2 = 4;
    if (has_param2()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param2());
    }

    // optional int64 param3 = 5;
    if (has_param3()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param3());
    }

    // optional int64 param4 = 6;
    if (has_param4()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param4());
    }

    // optional int64 param5 = 7;
    if (has_param5()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param5());
    }

    // optional int64 param6 = 8;
    if (has_param6()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->param6());
    }

  }
  if (_has_bits_[8 / 32] & (0xffu << (8 % 32))) {
    // optional string param7 = 9;
    if (has_param7()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::StringSize(
          this->param7());
    }

    // optional string stime = 10;
    if (has_stime()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::StringSize(
          this->stime());
    }

    // optional int64 parent = 11;
    if (has_parent()) {
      total_size += 1 +
        ::google::protobuf::internal::WireFormatLite::Int64Size(
          this->parent());
    }

  }
  if (!unknown_fields().empty()) {
    total_size +=
      ::google::protobuf::internal::WireFormat::ComputeUnknownFieldsSize(
        unknown_fields());
  }
  GOOGLE_SAFE_CONCURRENT_WRITES_BEGIN();
  _cached_size_ = total_size;
  GOOGLE_SAFE_CONCURRENT_WRITES_END();
  return total_size;
}

void EventSync::MergeFrom(const ::google::protobuf::Message& from) {
  GOOGLE_CHECK_NE(&from, this);
  const EventSync* source =
    ::google::protobuf::internal::dynamic_cast_if_available<const EventSync*>(
      &from);
  if (source == NULL) {
    ::google::protobuf::internal::ReflectionOps::Merge(from, this);
  } else {
    MergeFrom(*source);
  }
}

void EventSync::MergeFrom(const EventSync& from) {
  GOOGLE_CHECK_NE(&from, this);
  if (from._has_bits_[0 / 32] & (0xffu << (0 % 32))) {
    if (from.has_evid()) {
      set_evid(from.evid());
    }
    if (from.has_pid()) {
      set_pid(from.pid());
    }
    if (from.has_param1()) {
      set_param1(from.param1());
    }
    if (from.has_param2()) {
      set_param2(from.param2());
    }
    if (from.has_param3()) {
      set_param3(from.param3());
    }
    if (from.has_param4()) {
      set_param4(from.param4());
    }
    if (from.has_param5()) {
      set_param5(from.param5());
    }
    if (from.has_param6()) {
      set_param6(from.param6());
    }
  }
  if (from._has_bits_[8 / 32] & (0xffu << (8 % 32))) {
    if (from.has_param7()) {
      set_param7(from.param7());
    }
    if (from.has_stime()) {
      set_stime(from.stime());
    }
    if (from.has_parent()) {
      set_parent(from.parent());
    }
  }
  mutable_unknown_fields()->MergeFrom(from.unknown_fields());
}

void EventSync::CopyFrom(const ::google::protobuf::Message& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

void EventSync::CopyFrom(const EventSync& from) {
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool EventSync::IsInitialized() const {

  return true;
}

void EventSync::Swap(EventSync* other) {
  if (other != this) {
    std::swap(evid_, other->evid_);
    std::swap(pid_, other->pid_);
    std::swap(param1_, other->param1_);
    std::swap(param2_, other->param2_);
    std::swap(param3_, other->param3_);
    std::swap(param4_, other->param4_);
    std::swap(param5_, other->param5_);
    std::swap(param6_, other->param6_);
    std::swap(param7_, other->param7_);
    std::swap(stime_, other->stime_);
    std::swap(parent_, other->parent_);
    std::swap(_has_bits_[0], other->_has_bits_[0]);
    _unknown_fields_.Swap(&other->_unknown_fields_);
    std::swap(_cached_size_, other->_cached_size_);
  }
}

::google::protobuf::Metadata EventSync::GetMetadata() const {
  protobuf_AssignDescriptorsOnce();
  ::google::protobuf::Metadata metadata;
  metadata.descriptor = EventSync_descriptor_;
  metadata.reflection = EventSync_reflection_;
  return metadata;
}


// @@protoc_insertion_point(namespace_scope)

}  // namespace Message

// @@protoc_insertion_point(global_scope)
